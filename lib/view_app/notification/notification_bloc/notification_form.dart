...
import '../../../core/logic/parse_date.dart';
...
class _NotificationFormState extends State<NotificationForm> {
  ...
  void _scrollListener(int indexNotification) {
    if (_scrollControllers[indexNotification]!.position.pixels ==
        _scrollControllers[indexNotification]!.position.maxScrollExtent) {
      ...
      if (a < (_manageNotification.total ?? 0)) {
        ...
        context.read<NotificationBloc>().add(LoadMoreNotification(
            pageIndex: _pageIndexes[indexNotification]!,
          ...
          child: BlocListener<NotificationBloc, NotificationState>(
            listener: (context, state) {
              if (state is NotificationLoadingFullSplash) {
                ...
                setState(() {
                  _manageNotification.addMoreNotifications(
                      state.manageNotification.notifications ?? []);
                  index = state.index;
                });
              }
            },
  ...
  Widget _bodyNotification(ManageNotification manageNotification,
      int currentTypeNotification, NotificationState state) {
    if (manageNotification.notifications!.isEmpty) {
      if (state is NotificationLoadingFullSplash) return SizedBox();
      return const Center(
        child: Text("Empty!"),
      );
    } else {
      return ListView.builder(
          ...
          itemCount: manageNotification.notifications!.length +
              ((state is NotificationLoadingMoreSplash) ? 1 : 0),
          itemBuilder: (BuildContext context, index) {
            if (index < manageNotification.notifications!.length) {
              if (manageNotification.notifications![index].notifyType ==
                  'AUTOPROMO') {
                return _itemNotification(
                    FontAwesomeIcons.gift,
                    Colors.red,
                    manageNotification.notifications![index],
                    currentTypeNotification);
              } else if (manageNotification.notifications![index].notifyType ==
                  'DISPLAY') {
                return _itemNotification(
                    FontAwesomeIcons.building,
                    const Color(0xFF5757FE),
                    manageNotification.notifications![index],
                    currentTypeNotification);
              } else if (manageNotification.notifications![index].notifyType ==
                  'ACCUMULATE') {
  ...
  Widget _itemNotification(IconData icon, Color color,
      NotificationModel notification, int currentTypeNotification) {
        ...
        if (!(notification.isRead)!) {
          ...
          Navigator.push(context, MaterialPageRoute(builder: (context) {
            return DetailNotificationScreen(
              ...
              fromDate: notification.fromdate as String,
              toDate: notification.toDate as String,
            );
          })).then((value) {
            context.read<NotificationBloc>().add(RefreshNotification(
                indexNotificationBeforeNavigator: currentTypeNotification));
          });
        } else {
          Navigator.push(context, MaterialPageRoute(builder: (context) {
            return DetailNotificationScreen(
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                _contentItemNotification(
                    notification.title as String,
                    notification.fromdate as String,
                    notification.toDate as String,
  ...
  Widget _contentItemNotification(
      String title, String startDate, String endDate, bool isRead) {
    return Expanded(
      child: Column(
        ...
        children: [
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                                color: Colors.red.withOpacity(0.2), width: 4),
                          ),
                        ),
                        Container(
                          height: 8,
                          ...
                          decoration: BoxDecoration(
                            shape: BoxShape.circle,
                            border: Border.all(
                                color: const Color(0xFFD92332), width: 4),
                          ),
                        )
                      ],
                    )
          ...
          Text(
            "Từ ngày ${ParseDate.getDMY(startDate)} - ${ParseDate.getDMY(endDate)}",
            style: const TextStyle(fontSize: 12, color: Color(0xFFA4A4A8)),
          )
        ],
      ),
    );
