        ...
        child: SingleChildScrollView(
            ...
            child: BlocListener<NotificationBloc, NotificationState>(
              ...
              child: Row(
                children: [
                  _kindNotification(
                      "Tất cả", (countNotification?.totalAll ?? 0), 0, index),
                  _kindNotification("Trade khuyến mãi",
                      (countNotification?.totalTradePromotion ?? 0), 1, index),
                  _kindNotification("CT Trade khác",
                      (countNotification?.totalOtherTrade ?? 0), 2, index),
                  _kindNotification(
                      "Ghi chú", (countNotification?.totalNote ?? 0), 3, index),
                  _kindNotification(
                      "Khác", (countNotification?.totalOther ?? 0), 4, index),
                ],
              ),
            )));
  }

  Widget _kindNotification(String typeNotification, int quantityNotification,
      int indexNotification, int currentIndexNotification) {
    ...
    return GestureDetector(
      onTap: () {
        ...
        String typeNotification =
            FactoryTypeNotification.getTypeNotification(indexNotification);
        context.read<NotificationBloc>().add(SwitchKindNotification(
            indexSwitch: indexNotification,
              ...
              : Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: const BoxDecoration(
                        shape: BoxShape.circle, color: Color(0xFFD92332)),
                    child: Text(
